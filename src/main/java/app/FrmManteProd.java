...
public class FrmManteProd extends JFrame {
	...
	void llenaCombo() {
		...
		// combo 2.listado de proveedores
	...
	void registrar() {
		...
		String nombre = txtDescripcion.getText(); //txtCodigo.getText();
	...
	void listado() {
		...
		for (Producto p : lstProducto) {
			...
			txtSalida.append("Categoria : " + p.getIdcategoria()+ " - " + p.getObjCategoria().getDescripcion()) ;

			txtSalida.append("Estado : " + p.getEst_prod()) ;

			txtSalida.append("Id Proveedor : " + p.getIdproveedor()+ " - " + p.getObjProveedor().getNombre_rs()) ;
	...
	void buscar() {
		...
		//obtener un producto segun el codigo ingresado
		EntityManagerFactory fabrica = Persistence.createEntityManagerFactory("jpa_sesion01");
		EntityManager em = fabrica.createEntityManager();
		...
		//si el codigo no existe, muestra un mensaje, si no muestra los datos de producto
		if (p == null) {
			aviso("Codigo No existe", "Aviso del sistema", JOptionPane.ERROR_MESSAGE);
